 ...
 *
 * Preference order:
 *   1. OPENCODE_PROJECT_DIR (if it names an existing directory)
 *   2. CURSOR_PROJECT_DIR   (if it names an existing directory)
 *   3. CLAUDE_PROJECT_DIR   (if it names an existing directory)
 *   4. the process working directory
 */
function resolveProjectDir() {
  const fromOpenCode = process.env.OPENCODE_PROJECT_DIR;
  if (looksLikeDir(fromOpenCode)) {
    return fromOpenCode;
  }
